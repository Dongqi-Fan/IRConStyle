...
def spatial_similarity(x):
    # 1.把输入fm转化成B*C*HW
    # 2.先把HW平方再开方
    # 3.在C维度上，把HW加起来，此时维度变成B*C
    # 4.因为是算空间相似性，所以把B*C又拉成B*C*HW，此时C个HW都是一样的
    # 5.fm / B*C*HW，得出归一化后的每个通道，分母相当于算sum
    # 6.fm和归一化后的fm进行矩阵乘法，算出最终的通道相似度
    fm = x.cpu()
...
class ContrastLoss(nn.Module):
    ...
    def L1_forward(self, teacher, student, neg, blur_neg=None):
        ...
        :return:
        """
        # 这里的teacher, stu, neg都是由vgg输出的，且都是5个tensor的列表
        loss = 0
        for i in range(len(teacher)):
            neg_i = neg[i].unsqueeze(0)                               # 扩充为(1, batch_neg, c, h, w)
            neg_i = neg_i.repeat(student[i].shape[0], 1, 1, 1, 1)     # 扩充为(batch_stu, batch_neg, c, h, w)
