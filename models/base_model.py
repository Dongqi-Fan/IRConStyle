...
class BaseModel():
    ...
    def validation(self, dataloader, current_iter, tb_logger, save_img=False):
        ...
        """
        if self.opt['dist']:    # 是否采用了distribution(多GPU)
    ...
    def _update_best_metric_result(self, dataset_name, metric, val, current_iter, save=True):
    ...
    def model_ema(self, decay=0.999, only_neg_g=True):
    ...
    def update_learning_rate(self, current_iter, warmup_iter=-1):
        ...
        """
        if current_iter > 1:
            # 因为每个模块都有个schedulers，故schedulers是个列表，这里每个列表都step更新
            for scheduler in self.schedulers:
                scheduler.step()
        # set up warm-up learning rate
        if current_iter < warmup_iter and not self.use_Cosine:
            # get initial lr for each group (可以给不同模块设置不同学习率，同理加不同penalty)
    ...
    def load_network(self, net, load_path, strict=True, param_key='params', pretraining=False):
        """Load network.
