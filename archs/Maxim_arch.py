...
class CrossGatingBlock(nn.Module):  # input shape: n, c, h, w
    ...
    def forward(self, x, y):
        # y来自global_feature,就是bottleneck,downsample了三次
...
class Maxim(nn.Module):  # input shape: n, c, h, w
    ...
    """
    # num_supervision_scales：是用来设置用多少stage的输出拿来算损失，因为本文不仅仅拿最后的输出算损失
    # depth：UNet的深度
    # high_res_stages：具有高分辨率的stage，block更多
    def __init__(self, features=32, depth=3, use_bias=True, num_supervision_scales=int(3), lrelu_slope=0.2,
        ...
        self.stage_0_cross_gating_block_2 = CrossGatingBlock(x_features=3 * (2 ** 2) * self.features,
                                                             num_channels=4 * self.features,
                                                             block_size=self.block_size_hr if 2 < self.high_res_stages else self.block_size_lr,
                                                             grid_size=self.grid_size_hr if 2 < self.high_res_stages else self.block_size_lr,
                                                             cin_y=4 * self.features,
        ...
        self.stage_0_cross_gating_block_1 = CrossGatingBlock(x_features=3 * 2 * self.features,
                                                             num_channels=2 * self.features,
                                                             block_size=self.block_size_hr if 1 < self.high_res_stages else self.block_size_lr,
                                                             grid_size=self.grid_size_hr if 1 < self.high_res_stages else self.block_size_lr,
                                                             cin_y=4 * self.features,
                                                             upsample_y=True, use_bias=self.bias,
        ...
        self.stage_0_decoder_block_0 = UNetDecoderBlock(cin=2 * self.features, num_channels=self.features,
                                                        ...
                                                        use_global_mlp=self.use_global_mlp, dropout_rate=self.drop,
                                                        use_bias=self.bias)
        self.stage_0_output_conv_0 = nn.Conv2d((2 ** (0)) * self.features, self.num_outputs, kernel_size=(3, 3),
    ...
    def forward(self, x):
        ...
        for i in range(1, self.num_supervision_scales):
            shortcuts.append(nearest_downsample(x, 1. / (2 ** i)))    # shortcuts存储：x,x/2,x/4,x/8
        ...
        for i in range(self.num_supervision_scales):
            if i == 0:
                x_scale = self.stage_0_input_conv_0(shortcuts[i])  # 输出：features
                x_scales.append(x_scale)
            elif i == 1:
                x_scale = self.stage_0_input_conv_1(shortcuts[i])  # 输出：2 * features
                x_scales.append(x_scale)
            else:
                x_scale = self.stage_0_input_conv_2(shortcuts[i])  # 输出：4 * features
                x_scales.append(x_scale)
        ...
        for i in range(self.depth):
            ...
            enc_prev = None
            dec_prev = None
            # block的x和bridge的区别在于：x多比bridge经过了一个stride conv，即下采样；x_scale首先会和x进行concat
            if i == 0:
        ...
        for i in reversed(range(self.depth)):
            if i == 2:
                ...
                # Decoder block
                x = self.stage_0_decoder_block_2(x, bridge=signal)   # 用cross_block的输出作为额外输入
            elif i == 1:
                # get multi-scale skip signals from cross-gating block
                ...
                output = self.stage_0_output_conv_0(x) + shortcuts[i]
        return output
