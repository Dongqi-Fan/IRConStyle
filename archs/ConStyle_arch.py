...
class MyBaseEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        fea4 = self.pool(self.stage4(fea3)).squeeze(-1).squeeze(-1)
        out1 = self.mlp1(fea4)
        out2 = self.mlp2(out1)
        return [out1, out2], [fea1, fea2, fea3]


@ARCH_REGISTRY.register()
class ConStyle(nn.Module):
    """
    Build a MoCo model with: a query encoder, a key encoder, and a queue
    https://arxiv.org/abs/1911.05722
    """
    def __init__(self, Train, dim=128, K=65760, m=0.999, T=0.07):
        """
    ...
    def train_forward(self, im_q, im_k):
        ...
        return q, k[1], q_from_queue, logits, labels, feas
